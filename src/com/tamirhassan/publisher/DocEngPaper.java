...
public class DocEngPaper 
{
	public static void main(String[] args) throws Exception
	{
		// load file
		File file = new File("2591-h.htm");
		//File file = new File("98-h-orig.htm");
		//File file = new File("74-h.htm");
		Document doc = Jsoup.parse(file, "UTF-8", "");
		
		Elements paras = doc.getElementsByTag("p");
		
		System.out.println("no paras: " + paras.size());
		...
		ArrayList<String> shortParas = new ArrayList<String>();
		ArrayList<String> mediumParas = new ArrayList<String>();
		ArrayList<String> longParas = new ArrayList<String>();
		...
		for (Element para : paras)
		{
			...
			int numWords = 0;
			String trim = para.text().trim();
		    //if (trim.isEmpty())
		    //    return 0;
			if (!trim.isEmpty())
		    numWords = trim.split("\\W").length;
			
			int numChars = trim.length();
			
			if (numChars > 0)
			
			{
				if (numChars > maxChars) maxChars = numChars;
				if (numChars < minChars) minChars = numChars;
				...
				*/
				...
				// put in bins of ?100 chars
				int bin = numChars / 100;
				bins[bin] ++;
				
				if (numChars <= 300)
					shortParas.add(text);
				else if (numChars <= 750)
					mediumParas.add(text);
				else
					longParas.add(text);
		...
		System.out.println("no Paras: " + noParas);
		System.out.println("average chars: " + (totalChars/noParas));
		System.out.println("average words: " + (totalWords/noParas));
		...
		{
			cum += bins[i];
			System.out.println("bin: " + (i * 10) + " - " + (i * 10 + 9) + ": " + bins[i] + " cum: " + cum);
		}
		
		System.out.println();
		System.out.println("short paras (<=300): " + shortParas.size());
		System.out.println("medium paras (301-700): " + mediumParas.size());
		System.out.println("long paras (>750): " + longParas.size());
		
		System.out.println("total paras: " + allParas.size());
...
//		System.out.println("new container has items: " + newContainer.getContent().size());
		...
		*/
		...
		float fontSize = 18;
		System.out.println("results with allParas:");
		List<String> chosenParas = allParas;
		for (int i = 10; i <= 64; i ++)
		{
			//System.out.println("doceng *************************************************");
			//System.out.println("doceng i = " + i);
			
			// calculate width of paragraph in points
			float paraWidth = (fontSize * i);
			
			//System.out.println("paraWidth: " + paraWidth);
			//System.out.println("fontSize: " + fontSize);
			
			float sumFlexibility = 0;
			int paraCount = 0;
			
			
			for (String paraText : chosenParas)
			{
				PAFlexParagraph fp = new PAFlexParagraph(paraText, times, fontSize);
				
				List<PAFlexLayoutResult> paraLayouts = fp.layout(paraWidth, 100000, 1, true);
				
				//System.out.println("no layouts: " + paraLayouts.size());
				...
				for (PAFlexLayoutResult layout : paraLayouts)
				{
					int dummy = 0;
					//PAPhysTextBlock tb = (PAPhysTextBlock)layout.getItems().get(0);
					//System.out.println(layout);
					
					/*
					System.out.println("no lines: " + layout.getItems().size());
					
					for (Object line : layout.getItems())
						System.out.println(line);
					*/
					
					List<? extends PAPhysContainer> lines = layout.getItems();
					float demerits = (float) layout.getDemerits();
				...
				//System.out.println("smallest size: " + smallestLayoutSize);
				//System.out.println("largest size: " + largestLayoutSize);
				
				float flexibility = (((float)largestLayoutSize-smallestLayoutSize)/smallestLayoutSize);
				
				//float flexibility = (((float)largestLayoutSize-smallestLayoutSize)/largestLayoutSize);
				
				//System.out.println("flexibility: " + flexibility);
				
				sumFlexibility += flexibility;
				paraCount ++;
				
				
			}
			
			System.out.println("doceng i= " + i + " average flexibility: " + sumFlexibility / paraCount);
...
//			physPage.setHeight((float) (11.75 * 72));
			...
			PAFlexParagraph par6 =
					new PAFlexParagraph("", times, fontSize);
